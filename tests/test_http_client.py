...
import t99
import urllib3
from t99 import six, util

VALID_API_METHODS = ("get", "post", "delete")


class T99ClientTestCase(object):
    ...
    def request_mocks(self, mocker):
        ...
        for lib in self.REQUEST_LIBRARIES:
            request_mocks[lib] = mocker.patch("t99.http_client.%s" % (lib,))
        return request_mocks


class TestNewDefaultHttpClient(T99ClientTestCase):
    @pytest.fixture(autouse=True)
    def setup_warnings(self, request_mocks):
        original_filters = t99.http_client.warnings.filters[:]
        t99.http_client.warnings.simplefilter("ignore")
        yield
        t99.http_client.warnings.filters = original_filters

    def check_default(self, none_libs, expected):
        for lib in none_libs:
            setattr(t99.http_client, lib, None)

        inst = t99.http_client.new_default_http_client()

        assert isinstance(inst, expected)

    def test_new_default_http_client_urlfetch(self):
        self.check_default((), t99.http_client.UrlFetchClient)

    def test_new_default_http_client_requests(self):
        self.check_default(("urlfetch",), t99.http_client.RequestsClient)

    def test_new_default_http_client_pycurl(self):
        self.check_default(
            ("urlfetch", "requests"), t99.http_client.PycurlClient
        )

    def test_new_default_http_client_urllib2(self):
        self.check_default(
            ("urlfetch", "requests", "pycurl"),
            t99.http_client.Urllib2Client,
        )


class TestRetrySleepTimeDefaultHttpClient(T99ClientTestCase):
    ...
    def mock_max_delay(self, new_value):
        original_value = t99.http_client.HTTPClient.MAX_DELAY
        t99.http_client.HTTPClient.MAX_DELAY = new_value
        try:
            yield self
        finally:
            t99.http_client.HTTPClient.MAX_DELAY = original_value

    def test_sleep_time_exponential_back_off(self):
        client = t99.http_client.new_default_http_client()
    ...
    def test_initial_delay_as_minimum(self):
        ...
        initial_delay = t99.http_client.HTTPClient.INITIAL_DELAY
        self.assert_sleep_times(client, [initial_delay] * 5)

    def test_maximum_delay(self):
        client = t99.http_client.new_default_http_client()
        client._add_jitter_time = lambda t: t
        max_delay = t99.http_client.HTTPClient.MAX_DELAY
    ...
    def test_randomness_added(self):
        ...
        base_value = t99.http_client.HTTPClient.INITIAL_DELAY * random_value

        with self.mock_max_delay(10):
            expected = [
                t99.http_client.HTTPClient.INITIAL_DELAY,
                base_value * 2,
                base_value * 4,
                base_value * 8,
                base_value * 16,
...
class TestRetryConditionsDefaultHttpClient(T99ClientTestCase):
    ...
    def test_should_retry_on_t99_should_retry_true(self, mocker):
        client = t99.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"t99-should-retry": "true"}
    ...
    def test_should_retry_on_t99_should_retry_false(self, mocker):
        client = t99.http_client.new_default_http_client()
        client._max_network_retries = lambda: 1
        headers = {"t99-should-retry": "false"}
...
class TestHTTPClient(object):
    @pytest.fixture(autouse=True)
    def setup_t99(self):
        orig_attrs = {"enable_telemetry": t99.enable_telemetry}
    ...
    def test_sends_telemetry_on_second_request(self, mocker):
        class TestClient(t99.http_client.HTTPClient):
            pass
        ...
        assert "X-T99-Client-Telemetry" in args[2]

        telemetry = json.loads(args[2]["X-T99-Client-Telemetry"])
...
class ClientTestBase(object):
    ...
    def valid_url(self, path="/foo"):
        return "https://api.t99.com%s" % (path,)
    ...
    def test_exception(self, request_mock, mock_error):
        ...
        with pytest.raises(t99.error.APIConnectionError):
            self.make_request("get", self.valid_url, {}, None)


class RequestsVerify(object):
    def __eq__(self, other):
        return other and other.endswith("t99/data/ca-certificates.crt")


class TestRequestsClient(T99ClientTestCase, ClientTestBase):
    REQUEST_CLIENT = t99.http_client.RequestsClient
...
class TestRequestClientRetryBehavior(TestRequestsClient):
    ...
    def connection_error(self, session):
        ...
        def connection_error(given_exception):
            with pytest.raises(t99.error.APIConnectionError) as error:
    ...
    def test_handle_request_error_should_not_retry(
        ...
        assert "not verify T99's SSL certificate" in error.user_message
...
class TestUrlFetchClient(T99ClientTestCase, ClientTestBase):
    REQUEST_CLIENT = t99.http_client.UrlFetchClient
...
class TestUrllib2Client(T99ClientTestCase, ClientTestBase):
    REQUEST_CLIENT = t99.http_client.Urllib2Client
...
class TestPycurlClient(T99ClientTestCase, ClientTestBase):
    REQUEST_CLIENT = t99.http_client.PycurlClient
    ...
    def bio_mock(self, mocker):
        bio_patcher = mocker.patch("t99.util.io.BytesIO")
    ...
    def mock_error(self):
        def mock_error(mock):
            ...
            t99.http_client.pycurl.error = FakeException
            mock.perform.side_effect = t99.http_client.pycurl.error
    ...
    def check_call(self, request_mocks):
        def check_call(
            ...
            mock.setopt.assert_any_call(lib_mock.URL, t99.util.utf8(url))
...
class TestAPIEncode(T99ClientTestCase):
    def test_encode_dict(self):
        body = {"foo": {"dob": {"month": 1}, "name": "bat"}}

        values = [t for t in t99.api_requestor._api_encode(body)]
