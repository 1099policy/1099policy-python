...
from t99 import six, error


class TestT99Error(object):
    def test_formatting(self):
        err = error.T99Error(u"öre")
    ...
    def test_formatting_with_request_id(self):
        err = error.T99Error(u"öre", headers={"request-id": "123"})
    ...
    def test_formatting_with_none(self):
        err = error.T99Error(None, headers={"request-id": "123"})
    ...
    def test_repr(self):
        ...
        if six.PY2:
            assert (
                repr(err)
                == "T99Error(message=u'\\xf6re', http_status=None, "
                "request_id='123')"
            )
        else:
            assert (
                repr(err) == "T99Error(message='öre', http_status=None, "
                "request_id='123')"
            )
    ...
    def test_error_object_not_dict(self):
        err = error.T99Error("message", json_body={"error": "not a dict"})
        assert err.error is None


class TestT99ErrorWithParamCode(object):
    def test_repr(self):
        err = error.CardError(
            u"öre",
            param="cparam",
